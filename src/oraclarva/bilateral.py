...
class BilateralClosedLoopLarva:
    ...
    def __init__(
        ...
        *,
        ...
        self.segments = tuple(self.base["wave_segments_posterior_to_anterior"])
        ...
        for side_index, side in enumerate(SIDES):
            ...
            for segment_index, segment in enumerate(self.segments[:-1]):
                delay = round(
                    float(self.params["intersegmental_relay_delay_s"][segment])
                    / float(self.params["dt_s"])
                )
                synapses.append(Synapse(
                    self._channel_index(
        ...
        for projection in self.motor_identities:
            ...
            side_index = SIDES.index(projection.channel.side)
    ...
    def _validate_channel(
        ...
        if channel is None:
            return None
        if len(channel) != 2 or channel[0] not in valid_segments or channel[1] not in SIDES:
    ...
    def run(
        ...
        *,
        ...
        stimulus = stimulus or BilateralStimulus()
        p = self.params
        dt = float(p["dt_s"])
        ...
        rail_rest = [
            [
                self.body.bilateral_segment_length_m(
                    self.body_indices[segment], side
                )
                for side in SIDES
            ]
        ...
        stimulus_by_side = stimulus.by_side()

        for step in range(steps):
            time_s = step * dt
            external: dict[int, float] = {}
            if time_s < float(p["posterior_touch_duration_s"]):
                for side_index, side in enumerate(SIDES):
                    intensity = stimulus_by_side[side]
                    if intensity:
                        external[self.touch_offset + side_index] = (
                            intensity * float(p["posterior_touch_current_a"])
                        )
                difference = (
                    stimulus.left_touch_intensity
                    - stimulus.right_touch_intensity
                )
                if difference:
            ...
            for segment_index, segment in enumerate(self.segments):
                ...
                for side_index, side in enumerate(SIDES):
                    length = self.body.bilateral_segment_length_m(body_index, side)
            ...
            self.body.step(
                ...
                active_curvature_gain=float(
                    self.bilateral_params["active_curvature_gain"]
                ),
            ...
            maximum_abs_lateral = max(
                maximum_abs_lateral,
                *(abs(particle.position.y) for particle in self.body.particles),
            )
        ...
        heading = degrees(atan2(axis.y, axis.x))
    ...
    def _labels(self) -> tuple[str, ...]:
        labels = [f"environment_touch:{side}" for side in SIDES]
        labels.extend(f"rectified_sensory_difference:{side}" for side in SIDES)
