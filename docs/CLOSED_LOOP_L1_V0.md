...
The A1-A6 muscle layer preserves 358 left/right fiber identities, and the neural layer now instantiates all 58 resolved A1/A2 CATMAID motor identities. Only A1 is a causal identity proxy: A2 contains just two MN25 identities, three supplementary skeleton IDs are unresolved, and other segments retain aggregate motor pools. Individual fiber geometry is still not executed. The next iteration should export the physical trajectory to the continuous-surface viewer, then establish a shared Python/native parity fixture for the mobile core.
