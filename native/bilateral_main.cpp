...
std::string JoinNodes(const std::vector<oraclarva::BilateralVec3>& nodes) {
  std::ostringstream output;
  output << std::setprecision(17);
...
int main(int argc, char** argv) {
  try {
    if (argc < 2) {
      std::cerr << "usage: oraclarva-native-bilateral FIXTURE [OPTIONS]\n";
      return 2;
    }
    oraclarva::BilateralOptions options;
    for (int index = 2; index < argc; ++index) {
      const std::string option = argv[index];
      if (option == "--left" && index + 1 < argc) {
        options.left_stimulus = std::stod(argv[++index]);
      } else if (option == "--right" && index + 1 < argc) {
        options.right_stimulus = std::stod(argv[++index]);
    ...
    const oraclarva::BilateralFixture fixture =
        oraclarva::LoadBilateralFixture(argv[1]);
    const oraclarva::BilateralOutput result =
        oraclarva::RunBilateral(fixture, options);
    std::cout << std::setprecision(17);
    std::cout << "metadata\t" << fixture.schema << '\t' << fixture.model_id
              << '\t' << fixture.status << "\trelease_validated=false\n";
    std::cout << "summary\t" << result.displacement_x_um << '\t'
              << result.displacement_y_um << '\t'
              << result.heading_change_deg << '\t'
              << result.maximum_abs_lateral_um << '\t'
    ...
    for (std::size_t index = 0; index < fixture.wave_segments.size(); ++index) {
      std::cout << "wave\t" << index << '\t'
                ...
                << result.peak_activation[index][0] << '\t'
                << result.peak_activation[index][1] << '\t'
                << result.peak_shortening[index][0] << '\t'
                << result.peak_shortening[index][1] << '\n';
    }
    for (std::size_t index = 0; index < result.trajectory.size(); ++index) {
      const oraclarva::BilateralTrajectoryFrame& frame = result.trajectory[index];
      std::cout << "frame\t" << index << '\t' << frame.time_s << '\t'
                << JoinNodes(frame.nodes_m) << '\t'
                << JoinDoubles(frame.left_body_activation) << '\t'
                << JoinDoubles(frame.right_body_activation) << '\n';
    }
    return 0;
